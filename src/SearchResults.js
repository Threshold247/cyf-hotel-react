...
const SearchResults = (props) => {
  ...
  const headerArray = ["Id", "Title", "First Name", "Surname", "Email", "Room Id", "Check in Date","Check out Date", "Nights Stayed","Customer Profile"]; //table header
        ...
        <thead>
          <tr>
            ...
            <tbody>
              ...
              {bookings.map(({id,title,firstName,surname,email,roomId,checkInDate,checkOutDate})=>
                (<tr key={id}
					style={colorChg.includes(id)
						? {backgroundColor:blue}
						: {backgroundColor:blank}
					}
					onClick ={()=> handleColorChg(id)}>
                  ...
                  <td>{checkInDate}</td>
                  <td>{checkOutDate}</td>
				          <td style={centerMyText}>{nightStayed(checkOutDate,checkInDate)}</td>
                  <td><button type="button" className="btn-sm btn-primary" onClick={changeNumber}>Show Profile</button></td>
